...
//? How many presidests have we had?
console.log("How many Presidents?", presidentsArray.length);

//? Create a list of all presidents that are alive
const alivePresidents = presidentsArray.filter((president) => president.passed=== undefined)
;

console.log("Alive Presidents", alivePresidents);

// Dead Presidents 
const deadPresidents = presidentsArray.filter((president) => president.passed!==undefined);
...
let namedPresidents = alivePresidents.map(
...
// or can daisy chain
// const alivePresidents = presidentsArray.filter((president) => president.passed === undefined).map(
//     (president) => `${president.first} ${president.last}`);
...
deadPresidents.forEach((president) => {
        console.log(`${president.first} ${president.last} was ${president.passed - president.year} years old when he died.`)
    

})

// if grabbing from full array 
presidentsArray.forEach((president) => {
    if (presidentsArray.filter((president) => president.passed !== undefined)) {
        console.log(`${president.first} ${president.last} was ${president.passed - president.year} years old when he died.`)
    }
})
...
// !can also rename this case firstName to first
...
deadPresidents.forEach((president) => {
  //  Code goes in here with a console.log()
    // !Here extrapulating the info we need and assigning it
    let { passed, year, first, last } = president;
    //   passed  === president.passed
  let age = passed - year;
  console.log(`${first} ${last} was ${age} when he died`);
});